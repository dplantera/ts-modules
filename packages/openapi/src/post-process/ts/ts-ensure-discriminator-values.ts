  ...
  discriminatedUnions.forEach(({ node, discriminator }) => {
    ...
    return subTypes?.map((sub, idx) => {
      ...
      if (_.isNil(mappingValue) || _.isNil(subType) || _.isNil(propertyName)) {
        ...
        }' : ${JSON.stringify(mappingValue)}, mappings: ${JSON.stringify(subType)}`;
      }
      ...
      if (!propertyValue.isLiteral() && !propertyValue.isUnion()) {
        ...
        log.info(`fixed missing discriminator value on subtype ${subType.getName()}.${propertyName} - setting it with ${mappingValue}`);
      }
      // make union if allowed value literal does not match it
        ...
        log.info(
          `fixed missing discriminator value on subtype ${subType.getName()}.${propertyName} - extending ${propertyValue.getText()} with ${mappingValue}`
        );
      }
      // add allowed discriminator to union value if not present
        ...
        log.info(`fixed missing discriminator value on subtype ${subType.getName()}.${propertyName} - extending %j with ${mappingValue}`, unions);
      }
      return subType;
    });
  });
